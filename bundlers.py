...
def nr2h5( nr_strain_data, nr_meta_data, nr_init_data, output_path=None, verbose=False ):
    ...
    if ( not isinstance(nr_strain_data,dict) ) or ( not isinstance(nr_meta_data,dict) ) or ( not isinstance(nr_init_data, dict)):
    ...
    required_metadata_keys = [ 'Format','NR-group','type','name','alternative-names', 'NR-code', 'modification-date', 'point-of-contact-email',
			      'simulation-type','INSPIRE-bibtex-keys','license', 'Lmax', 'NR-techniques', 'files-in-error-series', 'comparable-simulation',		
				'production-run', 'object1','object2','mass1','mass2','eta', 'spin1x','spin1y','spin1z', 'spin2x','spin2y','spin2z',
	                  'LNhatx','LNhaty','LNhatz',  'nhatx','nhaty','nhatz', 'Omega', 'f_lower_at_1MSUN','eccentricity','PN_approximant', 'mean_anomaly' ]
